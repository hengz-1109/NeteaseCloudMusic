var audioOne, audioContext, audioSrc;
...
function initBackgroundOne() {

    audioOne, audioContext, audioSrc;
    analyser, analyserBufferLength;
    audioInfo = '';   
    MATHPI2 = Math.PI * 2;   
    w;
    h;  
    ...
    fftSize = 8192;//32, 64, 128, 256, 512, 1024, 2048, 4096, 8192, 16384, 32768   
    circleHolder = [];    
    time = 0;    
    colorInvertValue = 0;
...
function audioSetup( url ) {
  ...
  /*var newaudio = $('<audio>').html('<source src="'+url+'">').appendTo('#player');
  audioOne = newaudio[0];*/
  audioOne = new Audio();
  audioOne.src = url;
  audioOne.controls = false;
  audioOne.loop = true;
  audioOne.autoplay = false;
  audioOne.crossOrigin = 'anonymous';
  audioOne.volume = defultValmue;
  ...
  audioSrc = audioContext.createMediaElementSource( audioOne ); 
...
function userStart() {
  if ( audioOne.paused ) {
    ...
    if(bgOneInterval != undefined){
      clearInterval(bgOneInterval);
    }
    bgOneInterval = setInterval(function(){ animate(); },1000 / 144);
...
function setAudioOneCurrentTime(value) { 
  audioOne.currentTime = value;
}
  ...
  for ( var i = 0, l = circleHolder.length; i < l; i++ ) {
    ...
    for ( var j = 0, k = circleObj.segmentsOutside.length; j < k; j++ ) {
      ...
      if ( circleSegmentOutside.active === true ) {
        ...
        if ( j > 0 ) {		
          if ( aa === true && audioOne.paused === false ) {		
